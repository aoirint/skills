...
Make those non-Python bytes portable in the handoff: copy the exact font and renderer binary (and required adjacent runtime files when applicable) into a workspace-relative dependency snapshot, seal that relative path and hash, and verify from a receiver-supplied snapshot root. An absolute producer-machine path may remain provenance, but cannot be the only resolver for receiver-side verification.
