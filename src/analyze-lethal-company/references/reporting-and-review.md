...
Distinguish a clean full rerun, an additive extension, and a report-only rebuild. For a report-only change, keep certified numeric parents byte-identical, archive the prior report and downstream seals by rename, restore any analysis-stage media they own byte-for-byte, and rebuild only the report/handoff descendants. Do not rerun a full seed population merely because prose, headings, tables, or layout changed.
...
Keep short comparison tables together when practical; let long appendices split with repeated headers. Check full-size pages around every split. Avoid unconditional page breaks that create orphaned bullets or nearly blank pages. Re-number figures in display order and verify every body reference after reordering.
